...
async function searchAPIData(endpoint) {
  ...
  try {
    const response = await fetch(
      `${API_URL}search/${global.search.type}?query=${global.search.term}&api_key=${API_KEY}&language=en-US&page=${global.search.page}`,
    );
    if (!response.ok) {
      hideSpinner();
...
async function search() {
  ...
  //check to make sure the input field isn't empty
